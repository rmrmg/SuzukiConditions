...
def lookup(atoms, neighbour_list):
   ...
   offset = K.reshape(tf.range(batch_n, dtype=K.dtype(nei_in_lookup)), offset_shape)
   offset *= lookup_size
   #
...
def make_reduced_gcnn_module(prev_layer, reduced_graph_inputs, gcnn_config):
   ...
   features, nei = reduced_graph_inputs[:2]
   ...
   for it in range(N_it):
      ...
      L = Lambda(lambda x: lookup(x[0], x[1]))([H, nei])
      #print('WTF ',L)
      #curr_nei = Lambda(lambda x: K.sum(lookup(x[0], x[1]), axis=2)*x[2])([H, nei, normalization_factor])
      curr_nei = Lambda(lambda x: K.sum(x[0], axis=2)*x[1])([L, normalization_factor])
