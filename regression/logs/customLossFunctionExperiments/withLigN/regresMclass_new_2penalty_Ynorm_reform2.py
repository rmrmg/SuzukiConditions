...
def training(X,Y, model,lenx1, lenx2, Xax, Yax, nfolds=5, batchSize=20, nepoch=30):
        ...
        for epoch in range(nepoch):
            ...
            if True: #after full epoch
                ...
                for idxTestX, xtest in enumerate(Xtest):
                    ...
                    diffY.append( float(max(result2))-float(min(result2)) )
                ...
                folds[-1].append( (topNcount, statistics.mean(MAE), statistics.mean(diffY) ) )
    ...
    for i in range( len(folds[0])):
        ...
        ydiff = [x[2] for x in thisEpoch]
        print("epoch", i+1, [round(x,3) for x in avgTopN], [round(x,3) for x in stdTopN], statistics.mean(mae), statistics.stdev(mae), statistics.mean(ydiff) )
...
def customLoss(ytrue, ypred):
    ...
    for idx, i in enumerate(range(22, len(ypred), 30)):
        ...
        #print("YR", YrangePart)
        YrangePart = (50.0*ytrue[idx]) / (YrangePart+0.000001)
        Yrange += YrangePart
    #tf.print("RMSD", len(rmsd), rmsd, rmsd+diff, difftrue, diffpred)
    #return K.sum(K.log(ytrue) - K.log(ypred))
    res= (rmsd +abs(diff)+abs(Yrange))/len(ytrue)
    #print("RES", [res,] )
