...
#@tf.function
def training(X,Y, model,lenx1, lenx2, Xax, Yax, nfolds=5, batchSize=40, nepoch=30):
    ...
    for trainIdx, testIdx in kf5.split(X):
        ...
        for epoch in range(nepoch):
            ...
            if True: #after full epoch
                ...
                folds[-1].append( (topNcount, statistics.mean(MAE), statistics.mean(diffsY) ) )
    ...
    for i in range( len(folds[0])):
        ...
        for j in range( len(topN[0])):
            ...
            stdTopN.append( statistics.stdev([x[j] for x in topN]) )
        mae = [ x[1] for x in thisEpoch]
        diffy = [x[2] for x in thisEpoch]
        print("epoch", i+1, [round(x,3) for x in avgTopN],  [round(x,3) for x in stdTopN], "MAE", statistics.mean(mae), statistics.stdev(mae), "Ydiff", statistics.mean(diffy) )
...
def customLoss(ytrue, ypred):
    ...
    difftrue= ytrue[-2] - ytrue[-1]
    diffpred= ypred[-2] - ypred[-1]
    diff = abs(difftrue-diffpred)
...
if __name__ == "__main__":
    ...
    #raise
    print("DIM", len(X[0]), "args", sys.argv )
