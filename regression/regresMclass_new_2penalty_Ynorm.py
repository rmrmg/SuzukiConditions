...
def training(X,Y, model,lenx1, lenx2, Xax, Yax, nfolds=5, batchSize=20, nepoch=30):
    ...
    for i in range( len(folds[0])):
        ...
        print("epoch", i+1, [round(x,3) for x in avgTopN], [round(x,3) for x in stdTopN], "MAE", statistics.mean(mae), statistics.stdev(mae), "Ydiff", statistics.mean(ydiff) )
        epom = [fold[i] for fold in multipletResults]
        percentm = sum([x[0] for x in epom])/sum([ x[1] for x in epom])
        print("    pairs:", round(percentm,3), epom)
    #for i in range(1, 6):
