...
if __name__=='__main__':
   ...
   if args.graph:
      model_config = {'hidden_units': 20, 'hidden_units_mlp':100, 
                      'fgp_size':50, 'dropout': 0.2, 'lr':args.lr, 
                      'l2':1e-3, 'num_layers':3,
                      'activation':'relu', 'dropout_flag':False,
                      'concat':True, 'fgp':False}
   else:
   ...
   for train_idx, test_idx in kf.split(Y):
      ...
      for i,t in enumerate(Ytrain):
         logging.info('OUT %i'%i)
         p=baseline(model_config, Xtrain, t, Xtest, Ytest[i], graph=args.graph, train_config={'epochs':args.epochs})
