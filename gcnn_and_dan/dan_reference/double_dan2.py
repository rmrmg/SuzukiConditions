...
def make_dataset(test_config={}):
   ...
   x3, y3 = make_circles(size, noise=noise, factor=factor)
   return np.hstack([x,x2, x3]), np.hstack([y.reshape(-1,1), y2.reshape(-1,1), y3.reshape(-1,1)])
...
if __name__=='__main__':
   ...
   model_config = {'input_size':X.shape[1], 'output_size':Y.shape[1], 'layers':2, 'hidden':512}
   ...
   phi.fit(Xtrain, Yf_train, epochs=5, batch_size=100, verbose=True)
   ptest=phi.predict(Xtest)
   for i in range(Ytest.shape[1]):
     stats = tpr_tnr_stats(Ytest[:,i], ptest[:,i])
     logging.info('INIT: out:%i'%i+'  test: %5.3f %5.3f %5.3f bacc:%5.3f f:%5.3f'%tuple(stats))
