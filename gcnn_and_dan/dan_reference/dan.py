...
def Jpen(y_pred, y_true):
   ...
   return (1-log_phi_p)/(K.epsilon() + K.relu(log_phi_p-log_phi))
...
def make_dan_pair2(model_config):
   ...
   if model_config.get('opt','adam')=='adam':
      phi_wrapped.compile(optimizer=Adam(beta_1=0.5, beta_2=0.99, amsgrad=True, lr=model_config.get('lr',0.001)), loss=dummy_loss)
   else:
      phi_wrapped.compile(optimizer=SGD(lr=model_config.get('lr',0.001)), loss=dummy_loss)
...
def baseline(model_config, *train_stuff, train_config={}):
   ...
   tpr, tnr, acc, bacc = tpr_tnr_stats(ytest, p)
...
if __name__=='__main__':
   ...
   idx = (D>0) & (Y==1)
   Yfake[idx]=0
   logging.info('True: '+str(Y.sum()))
   logging.info('Fake: '+str(Yfake.sum()))
   logging.info('Corruption: %8.3f'%(1-Yfake.sum()/Y.sum()))
   ...
   for meta_epoch in range(args.epochs):
      ...
      #ptrain/=norm
      #ptest/=norm
      train_stats = tpr_tnr_stats(Ytrain, ptrain)
      test_stats = tpr_tnr_stats(Ytest, ptest)
      if meta_epoch%args.report==0:
         logging.info('ME: %5i loss:%8.4f  train_tpr_tnr: %5.3f %5.3f %5.3f  test_tpr_tnr: %5.3f %5.3f %5.3f'%(
         meta_epoch, loss, train_stats[0], train_stats[1], train_stats[3], test_stats[0], test_stats[1], test_stats[3]))
