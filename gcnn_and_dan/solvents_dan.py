...
if __name__=='__main__':
   ...
   model_config = {'input_size':X.shape[1], 'output_size':Y.shape[1], 'layers':5, 'hidden':128}
   ...
   if args.baseline:
      ...
      baseline(model_config, Xtrain, Ytrain, Xtest, Ytest, train_config={'epochs':args.epochs})
   ...
   phi.fit(Xtrain, Ytrain, epochs=5, batch_size=100, verbose=False)
   logging.info('order: tpr tnr bacc f')
   for meta_epoch in range(args.epochs):
      loss= make_dan_iteration(phi, phi_wr, d_mlp, disc, Xtrain, Ytrain)
