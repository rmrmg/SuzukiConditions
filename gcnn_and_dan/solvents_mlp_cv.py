...
from keras.layers import Input, Dense, Dropout, BatchNormalization, Lambda, Concatenate
...
def make_gcnn_classifier(input_shapes, output_shape, model_config):
   ...
   filters_shape1, max_atoms1 = graph1_shapes[1][1:]
   filters_shape2, max_atoms2 = graph2_shapes[1][1:]
   ...
   gcnn1_config['shapes']={'filters':graph1_shapes[1], 'max_atoms':max_atoms1, 'features':graph1_shapes[0]}
   gcnn2_config['shapes']={'filters':graph2_shapes[1], 'max_atoms':max_atoms2, 'features':graph2_shapes[0]}

   output1 = make_gcnn_module(graph1_inputs[0], graph1_inputs, gcnn1_config)
   output2 = make_gcnn_module(graph2_inputs[0], graph2_inputs, gcnn2_config)
...
def baseline(model_config, *train_stuff, train_config={}, graph=False):
   ...
   model.fit(xtrain, ytrain, sample_weight=w, epochs=epochs, batch_size=batch, verbose=True)
...
if __name__=='__main__':
   ...
   parser.add_argument('--log', type=str, default='mlp_cv.log')
   ...
   X, Y = prepare_data(desc=args.desc)
   if not args.graph:
      if args.standardize:
         X = (X-X.mean(axis=0))/(X.std(axis=0))
         X = np.nan_to_num(X)
      if args.join_base:
         _, B = prepare_data(out='base')
         X = np.hstack([X, B])
   else:
      graphs = [[XX[key] for key in ['X','filters','L','I','A']] for XX in X] #X_input, filters_input, nums_input, identity_input, adjacency_input 
      graphs = sum(graphs, [])
   ...
   if args.graph:
      model_config = {'hidden_units': 20, 'hidden_units_mlp':100, 
                      'fgp_size':50, 'dropout': 0.2, 'lr':1e-3, 
                      'l2':1e-3, 'num_layers':4,
                      'activation':'relu', 'dropout_flag':False,
                      'concat':True, 'fgp':True}
   else:
   ...
   for train_idx, test_idx in kf.split(Y):
      ...
      for i,t in enumerate(Ytrain):
         logging.info('OUT %i'%i)
         p=baseline(model_config, Xtrain, t, Xtest, Ytest[i], graph=args.graph)
      ...
      if args.separate:
         ...
         ks = categorical_stats(Ytest[0], baseline_ps[0])
      results.append(ks)   
   ...
   for i,x in enumerate(k_mean):
      logging.info('CV top-%i: %8.3f (%5.3f)'%(i,x,k_std[i]))
