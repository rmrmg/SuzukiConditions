...
def main(input_args=None):
    ...
    if args.data_name == 'suzuki':
        ...
        labels = ['Yield', 'M', 'L', 'B', 'S', 'A', 'id']
         ...
         datafile = 'suzuki_test.csv'
         class_dict = {'M': 37, 'L': 13, 'B': 9, 'S': 39}
         class_num = 103
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_test_data.npz'
    elif args.data_name == 'suzuki_rr_ours':
         datafile = 'test_suzuki_our_classes_translated.csv'
         class_dict = {'M': 37, 'L': 14, 'B': 7, 'S': 5}
         class_num = 68
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_test_data_ours.npz'
    elif args.data_name == 'suzuki_rr_ours_cv':
         datafile = f'test_f{args.fold}_suzuki_our_classes_translated.csv'
         class_dict = {'M': 37, 'L': 14, 'B': 7, 'S': 5}
         class_num = 68
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = f'rr_data_tstf{args.fold}_ours.npz'
         print(datafile, dataset_filename)
    elif args.data_name == 'suzuki_rr_ours_uspto':
         datafile = f'rr_data/gat_2_ml10_fakeT_valid{args.fold}.txt'
         class_dict = {'ML': 10, 'B': 7, 'S': 6}
         class_num = 27
         labels = ['Yield', 'ML', 'B', 'S', 'temperature']
         dataset_filename = f'rr_corr_data_uspto_ml10_ft_validation_f{args.fold}.npz'
         print(datafile, dataset_filename)
    elif args.data_name == 'suzuki_rr_ours_cv2':
         datafile = f'test_f{args.fold}_suzuki_our_classes2_translated.csv'
         class_dict = {'M': 37, 'L': 14, 'B': 7, 'S': 5}
         class_num = 68
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = f'rr_data_tstf{args.fold}_ours2.npz'
         print(datafile, dataset_filename)
    elif args.data_name == 'suzuki_rr_cond':
         datafile = 'test_suzuki_cond_class_translated.csv'
         class_dict = {'M': 37, 'L': 13, 'C':16}
         class_num = 70
         labels = ['Yield', 'M', 'L', 'C', 'temperature']
         dataset_filename = 'rr_test_data_cond.npz'
    elif args.data_name == 'suzuki_rr_cond_only':
         datafile = 'test_suzuki_cond_class_translated.csv'
         class_dict = {'C':16}
         class_num = 18
         labels = ['Yield', 'C', 'temperature']
         dataset_filename = 'rr_test_data_cond_only.npz'
         import pandas as pd
         ...
         datafile = 'test_suzuki_classes_translated.csv'
         class_dict = {'M': 37, 'L': 13, 'B':3, 'S':5}
         class_num = 63
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_test_data_classes.npz'
    elif args.data_name == 'suzuki_rr_classes_only':
         datafile = 'test_suzuki_classes_translated.csv'
         class_dict = {'M': 37, 'L': 13, 'B':3, 'S':5}
         class_num = 63
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_test_data_classes_only.npz'
         import pandas as pd
         d=pd.read_csv(datafile, sep=';')
         d['Yield']=0
         d['temperature']=0
    ...
    pickle.dump(y_pred, open(os.path.join(args.in_dir, f"pred{args.app}.pkl"), "wb"))
