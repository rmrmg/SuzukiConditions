...
def set_up_predictor(method, n_unit, conv_layers, class_num):
    ...
    if method == 'nfp':
        ...
        if chainer_chemistry.__version__.startswith('0.7'):
...
def main():
    ...
    if args.data_name == 'suzuki':
        ...
        labels = ['Yield', 'M', 'L', 'B', 'S', 'A', 'id']
    elif args.data_name == 'suzuki_rr':
         datafile = 'suzuki_train.csv'
         class_dict = {'M': 37, 'L': 13, 'B': 9, 'S': 39}
         class_num = 103
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_data.npz'
    elif args.data_name == 'suzuki_rr_ours':
         datafile = 'train_suzuki_our_classes_translated.csv'
         class_dict = {'M': 37, 'L': 14, 'B': 7, 'S': 5}
         class_num = 68
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_data_ours.npz'
    elif args.data_name == 'suzuki_rr_ours_cv':
         datafile = f'train_f{args.fold}_suzuki_our_classes_translated.csv'
         class_dict = {'M': 37, 'L': 14, 'B': 7, 'S': 5}
         class_num = 68
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = f'rr_data_tf{args.fold}_ours.npz'
    elif args.data_name == 'suzuki_rr_ours_cv2':
         datafile = f'train_f{args.fold}_suzuki_our_classes2_translated.csv'
         class_dict = {'M': 37, 'L': 14, 'B': 7, 'S': 5}
         class_num = 68
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = f'rr_data_tf{args.fold}_ours2.npz'
    elif args.data_name == 'suzuki_rr_ours_uspto':
         datafile = f'rr_data/gat_2_ml10_fakeT_train{args.fold}.txt'
         class_dict = {'ML': 10, 'B': 7, 'S': 6}
         class_num = 27
         labels = ['Yield', 'ML', 'B', 'S', 'temperature']
         dataset_filename = f'rr_corr_data_uspto_ml10_ft_f{args.fold}.npz'
    elif args.data_name == 'suzuki_rr_ours_only':
         datafile = 'train_suzuki_our_classes_translated.csv'
         class_dict = {'M': 37, 'L': 14, 'B': 7, 'S': 5}
         class_num = 68
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_data_ours0.npz'
         import pandas as pd
         ...
         d['M'] = 'MM'
         d['L'] = 'MM'
         d.to_csv(datafile.replace('translated','translated0'), sep=';', index=False)
         del d
         datafile = datafile.replace('translated','translated0')
    elif args.data_name == 'suzuki_rr_classes':
         datafile = 'train_suzuki_classes_translated.csv'
         class_dict = {'M': 37, 'L': 13, 'B':3, 'S':5}
         class_num = 63
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_data_classes.npz'
    elif args.data_name == 'suzuki_rr_classes_only':
         datafile = 'train_suzuki_classes_translated.csv'
         class_dict = {'M': 37, 'L': 13, 'B':3, 'S':5}
         class_num = 63
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_data_classes_only.npz'
         import pandas as pd
         d=pd.read_csv(datafile, sep=';')
         d['Yield']=0
         d['temperature']=0
         ...
         del d
         datafile = datafile.replace('translated','translated0')
    elif args.data_name == 'suzuki_rr_cond':
         datafile = 'train_suzuki_cond_class_translated.csv'
         class_dict = {'M': 37, 'L': 13, 'C':16}
         class_num = 70
         labels = ['Yield', 'M', 'L', 'C', 'temperature']
         dataset_filename = 'rr_data_cond.npz'
    elif args.data_name == 'suzuki_rr_cond_only':
         datafile = 'train_suzuki_cond_class_translated.csv'
         class_dict = {'C':16}
         class_num = 18
         labels = ['Yield', 'C', 'temperature']
         dataset_filename = 'rr_data_cond_only.npz'
         import pandas as pd
         d=pd.read_csv(datafile, sep=';')
         d['Yield']=0
         d['temperature']=0
         d['C'] = d.C.apply(lambda x: int(x[1:-1]) if len(x)>2 else '')
         minC = d[d.C!=''].C.min()
         d['C'] = d.C.apply(lambda x: f'M{x-minC}M' if x!='' else 'MM')
         d.to_csv(datafile.replace('translated','translated0'), sep=';', index=False)
         del d
         datafile = datafile.replace('translated','translated0')
    elif args.data_name == 'suzuki_rr_c100':
         datafile = 'suzuki_train_c100.csv'
         class_dict = {'M': 37, 'L': 13, 'B': 9, 'S': 39}
         class_num = 103
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_data_att100.npz'
    elif args.data_name == 'suzuki_rr_c50':
         datafile = 'suzuki_train_c50.csv'
         class_dict = {'M': 37, 'L': 13, 'B': 9, 'S': 39}
         class_num = 103
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_data_att50.npz'
    elif args.data_name == 'suzuki_rr_min':
         datafile = 'suzuki_train_min.csv'
         class_dict = {'M': 37, 'L': 13, 'B': 9, 'S': 39}
         class_num = 103
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_data_att_min.npz'
    elif args.data_name == 'suzuki_rr_c40':
         datafile = 'suzuki_train_c50.csv'
         class_dict = {'M': 37, 'L': 13, 'B': 9, 'S': 39}
         class_num = 103
         labels = ['Yield', 'M', 'L', 'B', 'S', 'temperature']
         dataset_filename = 'rr_data_att40.npz'
