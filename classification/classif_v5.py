...
def parseData(fn, normalize=False, removeSingleValueColumn=True):
    tabOryg=numpy.loadtxt(fn, delimiter='\t',  )
    #inputDim=len(tabOryg[0])
    X = tabOryg[:,2:-1] # dont get Y
    Ybase= tabOryg[:,0]
    X[numpy.isnan(X)] = 0
    Ybase = Ybase-1
    Ysolvent= tabOryg[:,1]
...
def makeModel(inputDim, solventClassNum, baseClassNum, wide1=40, wide2=10, act1='elu', act2='elu', act3='softmax'):
    init = tf.keras.initializers.RandomNormal(mean=2., stddev=1.)
    input_img = Input(shape=(inputDim,))
    hide1 = Dense(wide1, activation=act1)(input_img)
    hide9 = Dense(wide2, activation=act2)(hide1)
    solvent = Dense(solventClassNum, activation=act3, name='solvent', kernel_initializer=init)(hide9)
    base = Dense(baseClassNum, activation=act3, name='base')(hide9)
    model = Model(input_img, [solvent,base])
    optim = optimizers.Adam()# clipnorm=5, lr=0.00000001,  amsgrad=True )
...
if __name__ == "__main__":
    ...
    X,Ybase, Ysolvent = parseData(args.input, normalize=args.scale)
    print("input]", args)
